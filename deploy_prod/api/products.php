...
try {
    if (isset($_GET['slug'])) {
        $stmt = $db->prepare("SELECT id, title, slug, description, short_description, price, promotional_price, promotional_price_until, lowest_price_30_days, type, content_url, thumbnail_url, duration_label, long_description, benefits_json, meta_title, meta_desc, meta_image_url, canonical_url, noindex, stripe_price_id FROM products WHERE slug = :slug AND type != 'service' AND COALESCE(is_published, 1) = 1");
    ...
    $stmt = $db->query("SELECT id, title, slug, price, promotional_price, promotional_price_until, lowest_price_30_days, type, thumbnail_url, description, short_description, duration_label, meta_title, meta_desc, meta_image_url, canonical_url, noindex FROM products WHERE type != 'service' AND COALESCE(is_published, 1) = 1");
