...
if ($method === 'POST' && $action === 'products') {
    ...
    try {
        ...
        $stmt = $db->prepare('INSERT INTO products (id, title, slug, description, short_description, price, promotional_price, promotional_price_until, lowest_price_30_days, stripe_price_id, type, content_url, thumbnail_url, secondary_image_url, duration_label, long_description, benefits_json, meta_title, meta_desc, meta_image_url, canonical_url, noindex, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$id, $data['title'] ?? '', $data['slug'] ?? '', emptyToNull($data['description'] ?? null), emptyToNull($data['short_description'] ?? null), $data['price'] ?? 0, emptyToNull($data['promotional_price'] ?? null), emptyToNull($data['promotional_price_until'] ?? null), emptyToNull($data['lowest_price_30_days'] ?? null), emptyToNull($data['stripe_price_id'] ?? null), $data['type'] ?? '', emptyToNull($data['content_url'] ?? null), emptyToNull($data['thumbnail_url'] ?? null), emptyToNull($data['secondary_image_url'] ?? null), emptyToNull($data['duration_label'] ?? null), emptyToNull($data['long_description'] ?? null), serializeBenefitCards($data['benefits_json'] ?? null), emptyToNull($data['meta_title'] ?? null), emptyToNull($data['meta_desc'] ?? null), emptyToNull($data['meta_image_url'] ?? null), emptyToNull($data['canonical_url'] ?? null), parseBooleanFlag($data['noindex'] ?? false), parseBooleanFlag($data['is_published'] ?? true)]);
