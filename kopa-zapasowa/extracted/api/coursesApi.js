...
function userHasProductAccess(userId, productId) {
    const user = db.prepare('SELECT purchased_items, is_admin FROM users WHERE id = ?').get(userId);
    if (!user) return false;
    if (Number(user.is_admin) === 1) return true;
    return normalizePurchasedItems(user.purchased_items).includes(String(productId));
}

function requirePurchasedProductAccess(req, res, next) {
    const productId = req.accessProductId;
    if (!productId) {
        return res.status(500).json({ error: 'Brak kontekstu produktu do autoryzacji.' });
    }
    ...
    if (!userHasProductAccess(req.user.id, productId)) {
        return res.status(403).json({ error: 'Brak dostępu do treści tego produktu.' });
    }

    next();
}

function loadCourseAndAuthorize(req, res, next) {
    ...
    req.course = course;
    req.accessProductId = course.product_id;
    return requirePurchasedProductAccess(req, res, next);
}
...
// GET /api/courses/by-product/:productId - get course for a purchased product
router.get('/by-product/:productId', requireAuth, (req, res) => {
    try {
        const course = db.prepare('SELECT * FROM courses WHERE product_id = ?').get(req.params.productId);
        if (!course) return res.status(404).json({ error: 'Course not found' });
        req.accessProductId = course.product_id;
        if (!req.user.is_admin && !userHasProductAccess(req.user.id, course.product_id)) {
            return res.status(403).json({ error: 'Brak dostępu do treści tego produktu.' });
        }
...
router.post('/', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course created: ${id}`);
        res.json({ id, message: `Course created. ${build.message}`, publicBuild: build });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: err.message });
    }
...
router.put('/:id', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course updated: ${req.params.id}`);
        res.json({ message: `Course updated. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/:id', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course deleted: ${req.params.id}`);
        res.json({ message: `Deleted. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.post('/:id/modules', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course module created: ${id}`);
        res.json({ id, message: `Module created. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.put('/modules/:moduleId', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course module updated: ${req.params.moduleId}`);
        res.json({ message: `Module updated. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/modules/:moduleId', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course module deleted: ${req.params.moduleId}`);
        res.json({ message: `Deleted. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post('/modules/:moduleId/lessons', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course lesson created: ${id}`);
        res.json({ id, message: `Lesson created. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.put('/lessons/:lessonId', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course lesson updated: ${req.params.lessonId}`);
        res.json({ message: `Lesson updated. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/lessons/:lessonId', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`course lesson deleted: ${req.params.lessonId}`);
        res.json({ message: `Deleted. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post('/lessons/:lessonId/attachments', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`lesson attachment created: ${id}`);
        res.json({ id, message: `Attachment added. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete('/attachments/:attachmentId', requireAdmin, (req, res) => {
    try {
        ...
        const build = enqueuePublicBuild(`lesson attachment deleted: ${req.params.attachmentId}`);
        res.json({ message: `Deleted. ${build.message}`, publicBuild: build });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post('/progress', requireAuth, (req, res) => {
    try {
        ...
        if (!req.user.is_admin && !userHasProductAccess(userId, lesson.product_id)) {
            return res.status(403).json({ error: 'Brak dostępu do treści tego produktu.' });
        }
...
router.get('/progress/:courseId', requireAuth, loadCourseAndAuthorize, (req, res) => {
    try {
        const userId = req.user.id;
